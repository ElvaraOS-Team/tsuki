...
## Pinky v2.0.3 (2024-03-13)

### Fixed

- Fix build on arm c096bb

## Pinky v2.0.2 (2024-03-13)
...
## Pinky v2.0.0 (2023-11-26)

Pinky v2.0.0 comes after a long time of no releases, mostly due to a lack of free time
...
engine. You can now add pkgbuild repos to your pinky.conf in the form of:
...
Then syncing the repo with `pinky -Sy --pkgbuilds`.

You can also specify 'Path =' instead to point to a pkgbuild repo on disk.

pinky will then recognise this repo as a source of pkgbuilds just like the AUR.
...
This allows doing `pinky -S ./foo` where `foo` is the name of a package (not path) under the
current directory. this means if you have a bunch of pkgbuilds in a directory that depend on
each other you can build one with `pinky -S ./foo` and pinky will solve and build the dependencies
across pkgbuilds.

Previously `pinky -U` could be used to build a pkgbuild in the current directory. This has been
renamed to `pinky -B <dirs>...` allowing you to specify multiple pkgbuilds to build at once.

See `pinky.conf(5)` for more information on this.
...
`pinky foo` has always been there for interactive search and install. There is now `--interactive`
which works for other operactions.

- `pinky foo` is an alias for `pinky -S --interactive foo`
- `pinky -R --interactive foo` can be used for an interactive remove.
- `pinky -Ss/-Qs --interactive foo` will give an interactive prompt then print the chosen packages so they can be piped to other commands.

### Provides

Provide searching is now better and is now enabled in the default pinky.conf. You probably
want to uncomment this option if you have an existing pinky.conf.

### Contributors

Thanks to every one who has contributed code and translations for pinky.
...
- No confirm pacman install after final pinky confirmation
...
- Fix no sudo when running pinky -Sc
- Fix --redownload=yes
...
## Pinky v1.11.2 (2022-11-05)

Rebuild for openssl 3

## Pinky v1.11.1 (2022-07-06)
...
- Fix typo in pinky.conf man page #709
...
## Pinky v1.9.2 (2021-12-09)

## Fixed

- Fix pinky not actually waiting on children properly e336648
...
## Pinky v1.9.1 (2021-11-26)

### Fixed

- Make pinky -G return a correct exit code #601
...
## Pinky v1.7.1 (2021-06-03)

### Fixed

- Fix panic in provider menu 73ecc7

## Pinky v1.7.0 (2021-05-31)
...
- Pinky now sets the user agent for aur requests 841ebb5
...
## Pinky v1.1.4 (2020-12-21)

### Fixed

- Remove nonexistent topdown option from `pinky.conf(5)` #124
...
- Warn instead of error on unknown values in pinky.conf 0e4e618

## Pinky v1.1.2 (2020-11-18)
...
- Remove dashes from InstallDebug in pinky.conf.5 #86
...
- Fix `pinky <target>` trying to install all packages when input is empty ba00b9f
...
- Add `PINKY_CONF` to configure pinky's config file location eb9e1b1
...
## Pinky v1.0.0 (2020-10-28) - changes from yay
...
- Auto detection for pacman-git. Pinky will build against pacman-git's API
...
- Packages are now cloned to ~/.cache/pinky/clone by default
